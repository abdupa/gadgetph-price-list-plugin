document.addEventListener('DOMContentLoaded', function() {
    ...
    function populateAllSections() { 
        ...
        // 3. UPDATED LISTS (Showing top 5 items instead of 3)
        document.getElementById('budget-contenders').innerHTML = phoneData.filter(p=>p.price<=10000).sort((a,b)=>a.price-b.price).slice(0,5).map(renderContender).join('') || '<li class="py-2 text-xs text-gray-500">No phones found.</li>'; 
        document.getElementById('mid-range-contenders').innerHTML = phoneData.filter(p=>p.price>10000&&p.price<=25000).sort((a,b)=>a.price-b.price).slice(0,5).map(renderContender).join('') || '<li class="py-2 text-xs text-gray-500">No phones found.</li>'; 
        document.getElementById('premium-contenders').innerHTML = phoneData.filter(p=>p.price>25000&&p.price<=50000).sort((a,b)=>a.price-b.price).slice(0,5).map(renderContender).join('') || '<li class="py-2 text-xs text-gray-500">No phones found.</li>'; 
        document.getElementById('flagship-contenders').innerHTML = phoneData.filter(p=>p.price>50000).sort((a,b)=>b.price-a.price).slice(0,5).map(renderContender).join('') || '<li class="py-2 text-xs text-gray-500">No phones found.</li>'; 
