 ...
 * Updated to filter out "Watch" products from all lists.
 */
function spl_render_price_list_shortcode( $atts ) {
    ...
    // --- CACHE KEY UPDATE: v5 ---
    // We bumped this to v5 to force a refresh and remove existing 'watch' data from the cache.
    ...
    if ( false !== $cached_products ) {
        ...
        if ( $products_query->have_posts() ) {
            while ( $products_query->have_posts() ) {
                ...
                // --- FIX: EXCLUDE WATCHES ---
                // We check the title immediately. If it contains "watch", we skip it.
