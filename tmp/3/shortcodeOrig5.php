...
/**
 * Shortcode: [spl_price_drops brand="samsung" limit="3"]
 * Purpose: Powers the "📉 Latest Samsung Galaxy Price Drops" section.
 * Logic: Finds products where Sale Price < Regular Price, sorted by biggest savings.
 */
function spl_render_price_drops_shortcode( $atts ) {
    $atts = shortcode_atts( array(
        'brand' => '',
        'limit' => 3, // Default to top 3 deals
    ), $atts );

    $brand_slug = trim( strtolower( $atts['brand'] ) );
    $limit = intval( $atts['limit'] );

    // 1. Get all phones from cache (Fast)
    $transient_key = 'spl_all_phones_data_v5';
    $cached_products = get_transient($transient_key);

    if ( false === $cached_products ) { return ''; }

    // 2. Filter: Brand + Has Discount
    $discounted_products = array_filter( $cached_products, function($p) use ($brand_slug) {
        ...
        // Check if on sale (Regular Price > Current Price)
        return $p['regular_price'] > $p['price'];
    });

    // 3. Sort by Biggest Discount Amount (Descending)
    // This ensures the "Best" deals appear first, justifying the "Latest/Best" heading.
    ...
    <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
        <h3 class="font-bold text-gray-900 text-lg mb-4 flex items-center">
            📉 Latest <?php echo ucfirst($brand_slug); ?> Price Drops
        </h3>
        <ul class="space-y-0 text-sm">
            <?php foreach ( $top_drops as $phone ) : ?>
                <?php 
                    $savings = $phone['regular_price'] - $phone['price'];
                    $percent = round( ($savings / $phone['regular_price']) * 100 );
                ?>
                <li class="py-3 border-b border-gray-100 flex justify-between items-center last:border-0">
                    <div>
                        <a href="<?php echo esc_url($phone['productUrl']); ?>" class="font-semibold text-gray-800 hover:text-blue-600 block">
                            <?php echo esc_html($phone['name']); ?>
                        </a>
                        <span class="text-xs text-red-600 font-bold">
                            🔥 Save ₱<?php echo number_format($savings); ?> (<?php echo $percent; ?>% Off)
                        </span>
                    </div>
                    <div class="text-right">
                        <span class="block font-bold text-red-600 text-lg">
                            ₱<?php echo number_format($phone['price']); ?>
                        </span>
                        <span class="text-xs text-gray-400 line-through">
                            ₱<?php echo number_format($phone['regular_price']); ?>
                        </span>
                    </div>
                </li>
            <?php endforeach; ?>
