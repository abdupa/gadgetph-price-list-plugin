...
/**
 * The AJAX handler that processes a single batch.
 * UPDATED: Calculates and permanently caches Price Drops upon completion.
 */
function spl_ajax_run_rebuild_batch() {
    ...
    // 1. Initialization (Clear Cache on first run)
    if ($offset === 0) {
    ...
    if ( $products_query->have_posts() ) {
        while ( $products_query->have_posts() ) {
            ...
            if (stripos($product_title, 'watch') !== false) { continue; }
    ...
    if ($new_offset >= $total_products) {
        ...
        // --- NEW LOGIC: CACHE PRICE DROPS & DATE ---
        
        // A. Save the Date
        update_option('spl_manual_rebuild_date', current_time('mysql'));

        // B. Calculate Price Drops NOW (so the frontend doesn't have to)
        // We filter the massive $new_data array we just built.
        ...
        usort($drops_data, function($a, $b) {
            return ($b['regular_price'] - $b['price']) - ($a['regular_price'] - $a['price']);
        });

        // Save the top 50 drops globally. Shortcode will filter by brand later.
        update_option('spl_cached_price_drops', array_slice($drops_data, 0, 50));
        
        // -------------------------------------------

        wp_send_json_success([
            'status' => 'done',
            'message' => "Complete! {$total_products} products processed. Price Drops Calculated.",
            'percentage' => 100,
            'offset' => $new_offset
        ]);
    } else {
