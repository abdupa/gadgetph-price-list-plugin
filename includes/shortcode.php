...
//     $atts = shortcode_atts(array(
//         'id'    => '',
//         'title' => 'Quick Specs', // Default title
//     ), $atts, 'spl_quick_specs_list');
...
//     // --- BUTTON ---
//     $button_url = $product->get_product_url();
//     if (empty($button_url)) $button_url = get_post_meta($product_id, '_product_url', true);
    
//     $button_text = $product->get_button_text(); 
//     if (empty($button_text) && !empty($button_url)) {
//         if (strpos($button_url, 'lazada') !== false) $button_text = 'Check Price on Lazada';
//         elseif (strpos($button_url, 'shopee') !== false) $button_text = 'Check Price on Shopee';
//         elseif (strpos($button_url, 'amazon') !== false) $button_text = 'Check Price on Amazon';
//         else $button_text = 'Check Latest Price';
//     }
//     if (empty($button_url)) {
//          $button_url = get_permalink($product_id);
//          if (empty($button_text)) $button_text = 'View Details'; 
...
//     // NEW: Add the Heading Wrapper
//     $html = '<div class="spl-specs-container">';
    
//     // NEW: Add the Title (You can suppress it with title="none" in shortcode if needed)
//     if ($atts['title'] !== 'none') {
//         $html .= '<h5 class="spl-specs-title">' . esc_html($atts['title']) . '</h5>';
//     }
...
function spl_quick_specs_list_func($atts) {
    ...
    // UPDATED: Now uses H4
    if ($atts['title'] !== 'none') {
        $html .= '<h4 class="spl-specs-title">' . esc_html($atts['title']) . '</h4>';
    }
