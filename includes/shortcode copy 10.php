...
function spl_render_price_list_shortcode( $atts ) {
    ...
    if ( ! empty( $requested_brand ) ) {
        ...
        // Reset array keys so JSON output is a clean array, not an object with missing keys
    ...
    wp_localize_script( 'spl-main-js', 'priceListData', [
        ...
        'comparisons' => [], 
        'preSelectedBrand' => $requested_brand // Pass this to JS in case you want to lock the dropdown
    ]);
    
    // Determine dynamic Title based on filter
    $page_title = empty( $requested_brand ) 
        ? 'Comprehensive Smartphone Price List' 
        : ucfirst( $requested_brand ) . ' Price List';
    ...
    <div class="bg-gray-100 p-2 md:p-4 rounded-lg">
        ...
        <p class="text-gray-600">
            ...
            <?php else : ?>
                Browse the latest prices for <strong><?php echo esc_html( ucfirst( $requested_brand ) ); ?></strong> smartphones. 
                Use the search bar to find specific models.
            <?php endif; ?>
            The list updates instantly as you type&mdash;showing live prices pulled directly from our WooCommerce database.
        </p>
        ...
        <div class="bg-white p-4 md:p-6 rounded-xl card-shadow mb-8 sticky top-4 z-10 border-b border-gray-200">
         ...
         <?php endif; // End check for requested_brand ?>
