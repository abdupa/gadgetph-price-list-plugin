document.addEventListener('DOMContentLoaded', function() {
    console.log("Smartphone Price List Script v1.4 -- Fixed & Robust -- is running.");

    // Ensure data exists before running
    if (typeof priceListData === 'undefined' || !priceListData.phones) {
        console.error("Error: priceListData is missing or empty.");
        return;
    }
    ...
    function populateAllSections() { 
        ...
        const safeSetHTML = (id, htmlContent) => {
            const el = document.getElementById(id);
            if (el) el.innerHTML = htmlContent;
        };

        // 3. UPDATED LISTS - NOW CRASH PROOF
        // We calculate the lists first
        const budgetList = phoneData.filter(p=>p.price<=10000).sort((a,b)=>a.price-b.price).slice(0,3).map(renderContender).join('') || '<li class="py-2 text-xs text-gray-500">No phones found.</li>';
        const midList = phoneData.filter(p=>p.price>10000&&p.price<=25000).sort((a,b)=>a.price-b.price).slice(0,3).map(renderContender).join('') || '<li class="py-2 text-xs text-gray-500">No phones found.</li>';
        const premiumList = phoneData.filter(p=>p.price>25000&&p.price<=50000).sort((a,b)=>a.price-b.price).slice(0,3).map(renderContender).join('') || '<li class="py-2 text-xs text-gray-500">No phones found.</li>';
        const flagshipList = phoneData.filter(p=>p.price>50000).sort((a,b)=>b.price-a.price).slice(0,3).map(renderContender).join('') || '<li class="py-2 text-xs text-gray-500">No phones found.</li>';

        // Then we safely inject them
        safeSetHTML('budget-contenders', budgetList);
        safeSetHTML('mid-range-contenders', midList);
        safeSetHTML('premium-contenders', premiumList);
        safeSetHTML('flagship-contenders', flagshipList);
    ...
    // 2. Populate filters and sidebars
    populateBrandFilter();
    populateAllSections();
    
    // 3. Attach Event Listeners (WITH SAFEGUARDS)
    // NOTE: Changed 'keyup' to 'input' for instant search results
    if (searchInput) {
        searchInput.addEventListener('input', filterPhones); // 'input' is better than 'keyup' for instant reaction
    } else {
        console.warn("Search input ID 'search-input' not found.");
    }

    if (brandFilter) {
        brandFilter.addEventListener('change', filterPhones);
    } else {
        console.warn("Brand filter ID 'brand-filter' not found.");
    }

    if (priceFilter) {
        priceFilter.addEventListener('change', filterPhones);
    } else {
        console.warn("Price filter ID 'price-filter' not found.");
    }
});