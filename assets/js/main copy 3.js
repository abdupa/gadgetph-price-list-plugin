document.addEventListener('DOMContentLoaded', function() {
        ...
        if (regularPricePhp && regularPricePhp > pricePhp) {
            ...
            if (isPrimary) {
                return `<div class="text-right">
                            <del class="text-base font-normal text-gray-400">${originalPrice}</del>
                            <p class="text-2xl font-extrabold text-red-600">${salePrice}</p>
                        </div>`;
            } else {
                return `<div class="text-right leading-tight">
                            <del class="text-xs font-normal text-gray-400">${originalPrice}</del>
                            <p class="font-bold text-red-600">${salePrice}</p>
                        </div>`;
            }
        }
        if (isPrimary) {
                ...
                <div class="flex-shrink-0 md:w-2/6 md:text-center md:border-l md:pl-6">
                    <div class="flex md:flex-col items-center justify-between">
                        ...
                        <div class="mt-0 md:mt-4 flex flex-col items-stretch w-1/2 md:w-full">
                            <a href="${phone.productUrl}" class="inline-block bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-5 rounded-lg transition text-center">View Deal</a>
    ...
    // ** FIXED: Added the "Show all" link back. **
        ...
        if (remaining <= 0 || currentIndex >= totalItems) {
            ...
            loadMoreButtonContainer.innerHTML = `
                <div class="flex flex-col items-center space-y-3">
                    <button onclick="window.showNextPhones()" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-full shadow-lg">
                        Show next ${itemsToLoad} phones (${currentIndex}/${totalItems} shown)
                    </button>
                    <a href="javascript:void(0)" onclick="window.showAllPhones()" class="text-sm text-indigo-600 hover:text-indigo-800 font-semibold underline">
    ...
    // ** FIXED: Added the missing showAllPhones function. **
    ...
    function filterPhones() {
        ...
        // ** FIXED: Corrected the variable name from priceFilter to maxPrice. **
        ...
        currentData = phoneData.filter(phone => {
            ...
            // ** FIXED: Now using the correct variable 'maxPrice'. **
            if (maxPrice === 'all') {
                priceMatch = true;
            } else if (maxPrice === '150000') {
                priceMatch = phone.price > 50000;
            } else {
                priceMatch = phone.price <= parseInt(maxPrice);
            }
    ...
    function populateAllSections() {
        ...
        document.getElementById('budget-contenders').innerHTML = phoneData.filter(p=>p.price<=10000).sort((a,b)=>a.price-b.price).slice(0,3).map(renderContender).join('') || '<li>No phones in this range.</li>';
        document.getElementById('mid-range-contenders').innerHTML = phoneData.filter(p=>p.price>10000&&p.price<=25000).sort((a,b)=>a.price-b.price).slice(0,3).map(renderContender).join('') || '<li>No phones in this range.</li>';
        document.getElementById('premium-contenders').innerHTML = phoneData.filter(p=>p.price>25000&&p.price<=50000).sort((a,b)=>a.price-b.price).slice(0,3).map(renderContender).join('') || '<li>No phones in this range.</li>';
        document.getElementById('flagship-contenders').innerHTML = phoneData.filter(p=>p.price>50000).sort((a,b)=>b.price-a.price).slice(0,3).map(renderContender).join('') || '<li>No phones in this range.</li>';
