document.addEventListener('DOMContentLoaded', function() {
    ...
    // We use the 'priceListData' object passed from WordPress.
                ...
                <div class="flex-shrink-0 md:w-2/6 md:text-center md:border-l md:pl-6">
                    <div class="flex md:flex-col items-center justify-between">
                        ...
                        <div class="mt-0 md:mt-4 flex flex-col items-stretch w-1/2 md:w-full">
                            ...
                            <a href="${phone.productUrl}" class="text-sm text-indigo-600 hover:text-indigo-800 mt-2 text-center">Full Details</a>
                        </div>
                    </div>
                </div>
            </div>`;
    ...
    function updateLoadMoreButton(totalItems) {
        ...
        if (remaining <= 0 || currentIndex >= currentData.length) {
            loadMoreButtonContainer.innerHTML = '';
        } else {
            const itemsToLoad = Math.min(displayStep, remaining);
            loadMoreButtonContainer.innerHTML = `
                <button onclick="showNextPhones()" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-full shadow-lg">
